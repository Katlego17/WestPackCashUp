<div>
    <form wire:submit.prevent="submit">
        @csrf
        <div>
            <div class="container" style="text-align: center">
                <!--first row-->
                  <div class="row">
                    <div class="col-sm  border">
                      Franchise Logo
                      <br>
                      Franchise Name
                    </div>
                    <div class="col-sm border">
                      Daily Cash Up Summary
                      <br>
                      *To Be Completed At Store Level*
                      <hr>
                      Comments
                      <input type="textarea" class="form-control" wire:model="comments" style="">
                    </div>
                    <div class="col-sm border">
                      Receipt Cash Up Summary
                      <!--First form-->
                      <div class="row">
                        <div class="col-sm ">
                          Total G4S Deposit1
                        </div>
                        <div class="col-sm ">
                            <input type="text" class="form-control" wire:model="totalG4SDeposit1" style="">
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          Loose Change
                        </div>
                        <div class="col-sm ">
                            <input type="text" class="form-control" wire:model="looseChange" style="">
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          Total G4S Deposit2
                        </div>
                        <div class="col-sm ">
                            <input type="text" class="form-control" wire:model="totalG4SDeposit2" style="">
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          Chanage From Bank
                        </div>
                        <div class="col-sm ">
                            <input type="text" class="form-control" wire:model="changeFromBank" style="">
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          Petty Cash
                        </div>
                        <div class="col-sm ">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          Cheques
                        </div>
                        <div class="col-sm ">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          Coin/Gift vouchers
                        </div>
                        <div class="col-sm ">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          Return Vouchers
                        </div>
                        <div class="col-sm ">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          Promotional Vouchers
                        </div>
                        <div class="col-sm ">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          EFT
                        </div>
                        <div class="col-sm ">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          Total Cards
                        </div>
                        <div class="col-sm ">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm border">
                          Store Total
                        </div>
                        <div class="col-sm border">
                          R
                        </div>
                      </div>

                    </div>
                    <div class="col-sm border">
                      Card Statement Check
                      <!--Second form-->
                      <div class="row">
                        <div class="col-sm border">
                          Portable device
                        </div>
                        <div class="col-sm border">
                          Innervation
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm border">

                        </div>
                        <div class="col-sm border">

                        </div>
                      </div>
                      <!--START-->
                      @foreach ($questions as $index=>$question)
                        <div class="row">
                            <div class="col-sm border">
                                <input type="text" class="form-control" wire:model="portableDevice" style="">
                            </div>
                            <div class="col-sm border">
                                <input type="text" class="form-control" wire:model="innovation" style="">
                            </div>
                      </div>
                      @endforeach
                      <!--END-->
                      <div class="row">
                        <div class="col-sm border">
                          R 0
                        </div>
                        <div class="col-sm border">
                         R 0
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm border">
                         R 0
                        </div>
                        <div class="col-sm border">
                         R 0
                        </div>
                      </div>
                    </div>
                  </div>
                  <!--MID-->
                  <div class="row">
                    <div class="col-sm  border">
                      DATE
                      <br>
                      Totals per chashier
                      <div class="row">
                        <div class="col-sm  border">
                          Cashier
                        </div>
                        <div class="col-sm border">
                          Cash
                        </div>
                        <div class="col-sm  border">
                          Cheques
                        </div>
                        <div class="col-sm border">
                          EFT
                        </div>
                        <div class="col-sm  border">
                          Portable Devices
                        </div>
                        <div class="col-sm border">
                          Innervation
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm  border">
                          -
                        </div>
                        <div class="col-sm border">
                          -
                        </div>
                        <div class="col-sm  border">
                          -
                        </div>
                        <div class="col-sm border">
                          -
                        </div>
                        <div class="col-sm  border">
                          -
                        </div>
                        <div class="col-sm border">
                          -
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm  border">
                         Total
                        </div>
                        <div class="col-sm border">
                          R
                        </div>
                        <div class="col-sm  border">
                          R
                        </div>
                        <div class="col-sm border">
                          R
                        </div>
                        <div class="col-sm  border">
                          R
                        </div>
                        <div class="col-sm border">
                          R
                        </div>
                      </div>
                    </div>
                    <div class="col-sm border">
                      <div class="row">
                        <div class="col-sm  border">
                          Cash/Card payments for vouchers
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm ">
                          More contents to come on vouchers
                        </div>
                      </div>
                    </div>
                  </div>
                  <!--END-->
                  <div class="row">
                    <div class="col-sm  border">
                      Physical Cash Check
                      <div class="row">
                        <div class="col-sm  border">
                          Total G4S Deposits
                        </div>
                        <div class="col-sm border">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm  border">
                          Loose Change
                        </div>
                        <div class="col-sm border">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm  border">
                          Total cash per summary
                        </div>
                        <div class="col-sm border">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm  border">
                          Total cash per cashier
                        </div>
                        <div class="col-sm border">
                          R
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm  border">
                          Difference
                        </div>
                        <div class="col-sm border">
                         R
                        </div>
                      </div>
                    </div>
                    <div class="col-sm border"><!--STRETCH NEEDED-->
                      Petty Cash Reconciliation
                      <div class="row">
                        <div class="col-sm  border">
                          t
                        </div>
                        <div class="col-sm border">
                          t
                        </div>

                        <div class="col-sm  border">
                          t
                        </div>
                        <div class="col-sm border">
                          t
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm  border">
                          -
                        </div>
                        <div class="col-sm border">
                          -
                        </div>

                        <div class="col-sm  border">
                          -
                        </div>
                        <div class="col-sm border">
                          -
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm  border">
                          Total
                        </div>
                        <div class="col-sm border">

                        </div>
                        <div class="col-sm  border">

                        </div>
                        <div class="col-sm border">
                          R
                        </div>
                      </div>
                    </div>
                    <div class="col-sm border">
                      EFT Details
                      <div class="row">
                        <div class="col  border">
                          Date of Payments
                        </div>
                        <div class="col border">
                          Sales order number
                        </div>
                        <div class="col  border">
                          Amount
                        </div>
                      </div>
                      <div class="row">
                        <div class="col  border">
                          -
                        </div>
                        <div class="col border">
                          -
                        </div>
                        <div class="col  border">
                          -
                        </div>
                      </div>
                      <div class="row">
                        <div class="col  border">
                          Total
                        </div>
                        <div class="col border">

                        </div>
                        <div class="col  border">
                          R
                        </div>
                      </div>
                    </div>
                    <div class="col-sm border">
                      <div class="row">
                        <div class="col-sm  border">
                          Cash Office totals
                          <div class="row">
                            <div class="col-sm  border">
                              Floats Total
                            </div>
                            <div class="col-sm border">
                              -
                            </div>
                          </div>
                          <div class="row">
                            <div class="col-sm  border">
                              Change Boxes
                            </div>
                            <div class="col-sm border">
                              -
                            </div>
                          </div>
                          <div class="row">
                            <div class="col-sm  border">
                              Cash on hand
                            </div>
                            <div class="col-sm border">
                              -
                            </div>
                          </div>
                          <div class="row">
                            <div class="col-sm  border">
                              Total cash at store
                            </div>
                            <div class="col-sm border">
                              R
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm  border">
                         User Details
                          <div class="row">
                            <div class="col-sm  border">
                              Cash Up Details
                            </div>
                            <div class="col-sm border">
                              -
                            </div>
                          </div>
                          <div class="row">
                            <div class="col-sm  border">
                              Counting/Banking
                            </div>
                            <div class="col-sm border">
                              -
                            </div>
                          </div>
                          <div class="row">
                            <div class="col-sm  border">
                              Cash Up Summary
                            </div>
                            <div class="col-sm border">
                              -
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
              </div>
          </div>
          <div style="text-align: center; margin:1%;">
            <button type="submit" class="btn btn-success btn-lg btn-block">Submit</button>
          </div>
        </div>
        </div>

    </form>
</div>
